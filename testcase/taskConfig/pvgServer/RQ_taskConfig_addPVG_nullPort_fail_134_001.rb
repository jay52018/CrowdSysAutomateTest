#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_addPVG_nullPort_fail_134_001
# 用例标题: 添加端口号为空的pvg视频服务器
# 预置条件: 
#	1.管理员账号已进入任务配置页面
#	2.已弹出“添加PVG”窗口
# 测试步骤:
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_addPVG_nullPort_fail_134_001
	#使用管理员账号登录
	...
	#输入：名称、端口号、用户名、密码
	@driver.taskConfig_addPVG($pvgName,$pvgIP,'',$pvgAccount,$pvgPWD)
	#点击“确定”按钮
	@driver.taskConfig_addTaskServer_addPvg_submitBtn.click
	#检查弹出提示：“端口不能为空！”
	assert_equal($taskConfig_addPvgServer_nullPort_errCode,@driver.taskConfig_addTaskServer_errCode.when_present($waitTime).text,'添加端口为空的pvg，提示信息异常')
	#关闭添加窗口
