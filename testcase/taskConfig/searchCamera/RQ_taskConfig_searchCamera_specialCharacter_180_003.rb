#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_searchCamera_specialCharacter_180_003
# 用例标题: 搜索摄像机名包含下划线的摄像机
# 预置条件: 
#	1.管理员账号已进入任务配置页面
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_searchCamera_specialCharacter_180_003
	#使用管理员账号登录
	@driver.login($adminAccount,$adminPasswd)
	#登录后，检查当前页面存在assert_enterTaskConf_element元素，即认为登录成功
	assert(@driver.assert_enterTaskConfig_element.when_present($waitTime).present?,"登录失败，未跳转到指定页面")
	#输入关键字搜索摄像机
	@driver.taskConfig_keyWord_searchCamera_textField.when_present($waitTime).set('_')
	#检查返回包含关键字的摄像机名
