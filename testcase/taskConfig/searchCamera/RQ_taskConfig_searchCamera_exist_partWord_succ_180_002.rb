#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_searchCamera_exist_partWord_succ_180_002
# 用例标题: 部分关键字搜索存在的摄像
# 预置条件: 
#	1.管理员账号已进入任务配置页面
#	2.已添加名称为“标准视频”的摄像机
# 测试步骤:
#	1.在左边树列表上的搜索框中输入关键字：标准
#	2.已添加名称为“camera_1”的摄像机
# 预期结果:
#	1.在左边树列表上的搜索框中输入关键字：camera
#	2.在下拉框中点击“camera_1”
# 脚本作者: kangting
# 写作日期: 20161207
#=========================================================
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_searchCamera_exist_partWord_succ_180_002
	#使用管理员账号登录
	@driver.login($adminAccount,$adminPasswd)
	#登录后，检查当前页面存在assert_enterTaskConf_element元素，即认为登录成功
	assert(@driver.assert_enterTaskConfig_element.when_present($waitTime).present?,"登录失败，未跳转到指定页面")
	#输入关键字搜索摄像机
	@driver.taskConfig_keyWord_searchCamera_textField.when_present($waitTime).set('camera')
	#检查返回包含关键字的摄像机名
