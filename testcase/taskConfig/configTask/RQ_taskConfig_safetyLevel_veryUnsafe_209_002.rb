#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_safetyLevel_veryUnsafe_209_002
# 用例标题: 安全级别为很不安全的最小值等于最大值
# 预置条件: 
#	1.管理员账号已进入任务配置页面
#	2.已添加名称为“基础视频”的摄像机
#	3.已进入到“基础视频”的详细配置界面
# 测试步骤:
#	1.在安全级别为很不安全的最小值输入框输入：40
#	2.正确配置其他必填项
#	3.点击“保存”按钮
# 预期结果:
#	3.保存失败，弹出提示：“第4行最大值不能小于等于最小值！”
# 脚本作者: kangting
# 写作日期: 20161206
#=========================================================
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_safetyLevel_veryUnsafe_209_002
	#使用管理员账号登录
	...
	#点击安全级别为很不安全的最小值0
	@driver.taskConfig_safetySetting_data(0).click
	#在安全级别为安全的最小值输入框输入：“40”
	2.times { @driver.simlate_sendkeys(:backspace) }
	@driver.simlate_sendkeys(:numpad4)
	@driver.simlate_sendkeys(:numpad0)
	#点击“保存”按钮
	@driver.taskConfig_cameraConf_saveBtn.when_present($waitTime).click
	#弹出错误提示：第4行最大值不能小于等于最小值！
	assert_equal($taskConfig_safetySetting_veryUnsafeMin_errCode,@driver.taskConfig_safetySetting_errCode.when_present($waitTime).text,'设置安全级别为很不安全的最小值等于最大值，提示信息异常')
