#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_eventAlarmSet_gather_orangeLight_201_001
# 用例标题: 事件报警设置聚集事件闪橙光
# 预置条件: 
#	1.管理员账号已进入任务配置页面
#	2.已添加名称为“基础视频”的摄像机
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_eventAlarmSet_gather_orangeLight_201_001
	#使用管理员账号登录
	...
	#检查存在任务名称：基础视频，即认为成功跳转到任务详细配置页面
	assert_equal($basicTaskName,@driver.taskConfig_cameraConf_cameraName.when_present($waitTime).text,'点击配置失败，未跳转到任务详细配置页面')
	#在事件报警设置中，取消对聚集事件配置“闪橙光”策略（第二行第二列）
	@driver.taskConfig_eventAlarmSet_btn(2,2).when_present($waitTime).click
	#聚集一行中的“闪橙光”非高亮显示
	assert_equal("",@driver.taskConfig_eventAlarmSet_btn(2,2).attribute_value(:class),"还原失败！聚集一行中的闪橙光仍然高亮显示")
	#点击“保存”按钮
	@driver.taskConfig_cameraConf_saveBtn.when_present($waitTime).click
	#弹出提示：保存成功！
