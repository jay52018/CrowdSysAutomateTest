#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_gatherAreaDrawing_triggerValue_invalid_224_004
# 用例标题: 设置触发人群聚集事件阈值为非数字
# 预置条件: 
#	1.管理员账号已进入任务配置页面
#	2.已添加名称为“基础视频”的摄像机
#	3.已进入到“基础视频”的详细配置界面
# 测试步骤:
#	1.在触发人群聚集事件阈值输入框输入：a*?@
# 预期结果:
#	1.输入“a*?@”失败
# 脚本作者: kangting
# 写作日期: 20161209
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_gatherAreaDrawing_triggerValue_invalid_224_004
	#使用管理员账号登录
	...
	#输入触发聚集事件阈值：a*?@
	@driver.taskConfig_gatherAreaDraw_triggerThreshold_textField.set('a*?@')
	#输入“a*?@”失败
	assert_equal('',@driver.taskConfig_gatherAreaDraw_triggerThreshold_textField.value,'错误！可设置触发聚集事件阈值为非法字符')
	#点击“取消”按钮
