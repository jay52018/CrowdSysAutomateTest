#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_gatherAreaDrawing_triggerValue_null_224_005
# 用例标题: 设置触发人群聚集事件阈值为空
# 预置条件: 
#	1.管理员账号已进入任务配置页面
#	2.已添加名称为“基础视频”的摄像机
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_gatherAreaDrawing_triggerValue_null_224_005
	#使用管理员账号登录
	...
	#设置触发聚集事件阈值为空
	@driver.taskConfig_gatherAreaDraw_triggerThreshold_textField.set(nil)
	#点击“保存”按钮
	@driver.taskConfig_cameraConf_saveBtn.when_present($waitTime).click
	#弹出错误提示：触发人群聚集事件阈值不能为空 ！
	assert_equal($taskConfig_configureTask_gather_nullTriggerThreshold_errCode,@driver.taskConfig_cameraConf_message.when_present($waitTime).text,'设置触发聚集事件阈值为空，提示信息异常')
	#点击“取消”按钮
