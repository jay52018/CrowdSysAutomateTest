#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_taskConfig_safetyLevel_verySafe_206_005
# 用例标题: 安全级别为很安全的最小值大于安全级别为安全的最大值
# 预置条件: 
#	1.管理员账号已进入任务配置页面
#	2.已添加名称为“基础视频”的摄像机
#	3.已进入到“基础视频”的详细配置界面
# 测试步骤:
#	1.在安全级别为很安全的最小值输入框输入：81
#	2.正确配置其他必填项
#	3.点击“保存”按钮
# 预期结果:
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_taskConfig_safetyLevel_verySafe_206_005
	#使用管理员账号登录
	...
	#在安全级别为很安全的最小值输入框输入：“81”
	2.times { @driver.simlate_sendkeys(:backspace) }
	@driver.simlate_sendkeys(:numpad8)
	@driver.simlate_sendkeys(:numpad1)
	#点击“保存”按钮
	@driver.taskConfig_cameraConf_saveBtn.when_present($waitTime).click
	#弹出错误提示：第1行的最小值必须等于第2行的最大值！
	assert_equal($taskConfig_safetySetting_verySafeMin_safeMax_diff_errCode,@driver.taskConfig_safetySetting_errCode.when_present($waitTime).text,'设置安全级别为很安全的最小值大于安全级别为安全的最大值，提示信息异常')
