#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_userCenter_userManage_addNullUserId_fail_053_003
# 用例标题: 添加空用户名
# 预置条件: 
#	1.管理员已进入用户管理界面
# 测试步骤:
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_userCenter_userManage_addNullUserId_fail_053_003
	#使用管理员账号登录系统
	...
	#提示“用户名不能为空”
	assert_equal($userManage_addUser_NullUserId_errCode,@driver.userCenter_userManage_addUser_errCode.when_present($waitTime).text,"添加空用户名的用户，提示信息异常")
	#关闭“添加用户”窗口
	@driver.close_currWindow_with_X.click
	#用户列表中无用户姓名为“新用户”的用户信息
	assert_equal('false',@driver.userCenter_userManage_userListInf_index(5).present?.to_s,"用户列表中存在用户姓名为#{$newUserName}的用户信息")
	#注销系统，确认:0;取消：1
	@driver.logout()
  end
