#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: RQ_userCenter_userManage_addExistUserName_053_002
# 用例标题: 添加已存在的用户姓名
# 预置条件: 
#	1.管理员已进入用户管理界面
#	2.已存在用户名为：who 和用户姓名为：新用户 的用户
# 测试步骤:
#	1.点击“+添加新用户”
#	2.在“用户名”输入框输入：user1
#	3.在“用户姓名”输入框输入：新用户
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_userCenter_userManage_addExistUserName_053_002
	#使用管理员账号登录系统
	...
	#提示“用户添加成功！”
	assert_equal($userManage_addUser_succMessage,@driver.userCenter_userManage_addUser_succMessage.when_present($waitTime).text,"成功添加用户提示信息异常")
	...
	#再次添加用户名为：user1和用户姓名为：新用户 的用户
	@driver.userCenter_userManage_addUserBtn.click
	@driver.userCenter_userManage_addUserIdField.set("user1")
	@driver.userCenter_userManage_addUserNameField.set($newUserName)
	@driver.userCenter_userManage_submitAddUser_btn.click
	#提示“用户姓名不能重复”
	assert_equal($userManage_addUser_sameUserName_errCode,@driver.userCenter_userManage_addUser_errCode.when_present($waitTime).text,"添加已存在用户姓名的用户，提示信息异常")
	#关闭“添加用户”窗口
	@driver.close_currWindow_with_X.click
	#初始化用户名列表数据
	@driver.resore_defaultUser
	#检验用户列表中无用户名为“user1”，用户姓名为“新用户”的用户信息
