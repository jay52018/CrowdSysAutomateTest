#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID:RQ_userCenter_filterTrace_admin_modifyVideoService_073_017
# 用例标题: 过滤管理员的修改视频服务器日志
# 预置条件: 
#	1.使用管理员账号产生修改视频服务器动作
#	2.管理员已进入操作日志界面
# 测试步骤:
#	1.点击“用户”下拉框，勾选“系统管理员”
#	2.点击“动作”下拉框，勾选“修改视频服务器”动作
#	3.进行提交
#	4.查看日志列表
# 预期结果:
#	4.成功过滤，日志列表中只显示系统管理员的修改视频服务器操作日志信息
# 脚本作者: kangting
# 写作日期: 20161222
#=========================================================
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_userCenter_filterTrace_admin_modifyVideoService_073_017
	#清空所有操作日志
	...
	#将名称修改为：“2.32”
	@driver.taskConfig_editTaskServer_serverName_textField.set('2.32')
	#点击“保存”按钮
	@driver.taskConfig_editTaskServer_saveBtn.click
	#记录修改视频服务器的时间
	modifyPvgNameTime = @driver.time_now
	#检查弹出提示：“修改成功！”
	...
	#检查左边树列表中存在名称为“.2.32”的pvg，即认为pvg视频服务器名称修改成功
	assert(@driver.taskConfig_treeList_videoName('2.32').when_present($waitTime).present?,'修改视频服务器名称失败，左边树列表中未同步')
	
	#删除pvg，初始化
	@driver.taskConfig_deleteVideoServer('2.32',0,1)
	#弹出提示删除成功 
	...
	@driver.userCenter_filterOperateLog_withAction('修改视频服务器')
	
	#查看操作日志信息列表中的日志是否正确
	#判断操作日志记录时间与实际操作时间的偏差
	operateLog_recordTime = @driver.operateLog_recordTime(0)
	@driver.logTime_operateTime_deviation(modifyPvgNameTime,operateLog_recordTime,'modifyPvgNameTime')
	...
	#断言操作日志列表中的动作信息
	assert_equal("修改视频服务器，从[#{$pvgName}]修改为[2.32]成功",@driver.userCenter_trace_operateLogList_index(3).when_present($waitTime).text,'动作与操作日志中不一致！')
