#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID:RQ_userCenter_filterTrace_admin_deletePVG_073_015
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_userCenter_filterTrace_admin_deletePVG_073_015
	#清空所有操作日志
	...
	@driver.userCenter_filterOperateLog_withAction('删除pvg视频服务器')
	
	#查看操作日志信息列表中的日志是否正确
	#判断操作日志记录时间与实际操作时间的偏差
	operateLog_recordTime = @driver.operateLog_recordTime(0)
	@driver.logTime_operateTime_deviation(deletePvgTime,operateLog_recordTime,'deletePvgTime')
	...
	#断言操作日志列表中的动作为：删除pvg视频服务器[172.17.2.32]成功
	assert_equal("删除pvg视频服务器[#{$pvgName}]成功",@driver.userCenter_trace_operateLogList_index(3).when_present($waitTime).text,'动作与操作日志中不一致！')
