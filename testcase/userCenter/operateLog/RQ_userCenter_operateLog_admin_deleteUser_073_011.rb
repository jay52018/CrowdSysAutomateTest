#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID:RQ_userCenter_operateLog_admin_deleteUser_073_011
# 用例标题: 过滤管理员的删除用户日志
# 预置条件: 
#	1.使用管理员账号产生删除用户动作
#	2.管理员已进入操作日志界面
...
#	2.点击“动作”下拉框，勾选“删除用户”动作
#	3.进行提交
#	4.查看日志列表
# 预期结果:
#	4.成功过滤，日志列表中只显示系统管理员的删除用户操作日志信息
# 脚本作者: kangting
# 写作日期: 20161026
#=========================================================
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_userCenter_operateLog_admin_deleteUser_073_011
	#清空所有操作日志
	...
	#删除新增的用户,确认删除用户信息列表中第三行的用户
	@driver.userManage_deleteUser(4,0)
	#记录删除用户时间
	deleteUserTime = @driver.time_now
	#提示：删除成功
	...
	@driver.userCenter_filterOperateLog_withAction('删除用户')
	
	#查看操作日志信息列表中的日志是否正确
	#判断操作日志记录时间与实际操作时间的偏差
	operateLog_recordTime = @driver.operateLog_recordTime(0)
	@driver.logTime_operateTime_deviation(deleteUserTime,operateLog_recordTime,'deleteUserTime')
	...
	#断言操作日志列表中的动作为：删除用户[who|新用户]成功
	assert_equal('删除用户[who|新用户]成功',@driver.userCenter_trace_operateLogList_index(3).when_present($waitTime).text,'动作与操作日志中不一致！')
	#注销系统，确认:0;取消：1
	@driver.logout()
  end
