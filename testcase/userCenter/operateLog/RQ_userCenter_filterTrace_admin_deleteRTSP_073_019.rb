#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID:RQ_userCenter_filterTrace_admin_deleteRTSP_073_019
# 用例标题: 过滤管理员的删除RTSP视频日志
# 预置条件: 
#	1.使用管理员账号产生删除RTSP视频动作
#	2.管理员已进入操作日志界面
# 测试步骤:
#	1.点击“用户”下拉框，勾选“系统管理员”
#	2.点击“动作”下拉框，勾选“删除RTSP视频”动作
#	3.进行提交
#	4.查看日志列表
# 预期结果:
#	4.成功过滤，日志列表中只显示系统管理员的删除RTSP视频操作日志信息
# 脚本作者: kangting
# 写作日期: 20161222
#=========================================================
...
class CrowdSystemTest < MiniTest::Unit::TestCase
  ...
  def test_RQ_userCenter_filterTrace_admin_deleteRTSP_073_019
	#清空所有操作日志
	...
	#记录删除RTSP视频服务器的时间
	deleteRtspTime = @driver.time_now
	#弹出提示删除成功 
	...
	@driver.userCenter_filterOperateLog_withAction('删除rtsp视频')
	
	#查看操作日志信息列表中的日志是否正确
	#判断操作日志记录时间与实际操作时间的偏差
	operateLog_recordTime = @driver.operateLog_recordTime(0)
	@driver.logTime_operateTime_deviation(deleteRtspTime,operateLog_recordTime,'deleteRtspTime')
	...
	#断言操作日志列表中的动作为：删除rtsp视频[RTSP]成功
	assert_equal("删除rtsp视频[#{$rtspName}]成功",@driver.userCenter_trace_operateLogList_index(3).when_present($waitTime).text,'动作与操作日志中不一致！')
